...
namespace ImportExcelSql.Controllers
{
    public class Import2Controller : Controller
    {
                ...
                else
                {
                    ...
                    for (int row = 2; row <= rowCount; row++)
                    {
                        model.CarListViewModel.Add(new CarViewModel
                        {
                            ...
                            //horsePower = (worksheet.Cells[row, 6].Value ?? string.Empty).ToString().Trim(),
                            //price = (worksheet.Cells[row, 7].Value ?? string.Empty).ToString().Trim(),
                        });
                    }
                }
            }
        ...
        //public ActionResult ImportOnly(FormCollection form)
        {
            ...
            foreach (var student in model.CarListViewModel)
            {
                list.Add(new Car
                {
                    //StudentId = (int)(Double)worksheet.Cells[row, 1].Value,
                    carName = student.carName,
                    doorNumber = student.doorNumber
                });
            }
