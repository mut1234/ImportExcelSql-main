...
namespace ImportExcelSql.Controllers
{
    public class CarController : Controller
    {
        ...
        public async Task<ActionResult> File(FileUploadViewModel model)
        {
            ...
            string fileName = Guid.NewGuid().ToString() + Path.GetExtension(model.XlsFile != null ? model.XlsFile.FileName : model.CsvFile.FileName);
            string filePath = Path.Combine(rootFolder, fileName);

            // Save the uploaded file
            if (model.XlsFile != null)
            {
                // Save the uploaded XLS file
                using (FileStream fileStream = new FileStream(filePath, FileMode.Create))
                {
                    await model.XlsFile.CopyToAsync(fileStream);
                }
                ...
                using (ExcelPackage package = new ExcelPackage(new FileInfo(filePath)))
                {
                    ...
                    if (worksheet == null)
                    {
                        // Handle the case where the worksheet is null, return or display an error message
                        ModelState.AddModelError(string.Empty, "Excel file does not contain any worksheets.");
                        return View(model);
                    }
