...
namespace ImportExcelSql.Service
{
    public class CarService
    {
        ...
        */
        ...
        private string ConvertToNumber(int number)
        {
            switch (number)
            {
                case 1:
        ...
        */
        public List<Car> FilterCars(FilteredCarsViewModel filter)
        {
         ...
         (filter.MinCylinders == null || ConvertToString(car.numberOfCylinders) >= filter.MinCylinders) &&
         (filter.MaxCylinders == null || ConvertToString(car.numberOfCylinders) <= filter.MaxCylinders) &&
         (filter.MinDoors == null || ConvertToString(car.doorNumber) >= filter.MinDoors) &&
         (filter.MaxDoors == null || ConvertToString(car.doorNumber) <= filter.MaxDoors))
        .ToList();
